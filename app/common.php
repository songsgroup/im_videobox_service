...
if (!function_exists('dir_archive')) {
    /**
     * 目录归档为Zip文档
     * @param string $dir 目录
     * @param string $file 生成文件路径
     * @param string $temppre 临时文件前缀 $file为空时
     */
    function dir_archive(string $dir,string $file='',string $temppre=''):string
    {
        if (!is_dir($dir)) {
            throw new Exception('目录不存在');
        }
        if (!$file) {
            $file = tempnam(sys_get_temp_dir(), $temppre);
        }
        $zip = new \ZipArchive;
        ...
        foreach ($files as $fileinfo) {
            if (!$fileinfo->isDir()) {
                ...
                $zip->addFile($path,str_replace(DS,'/',substr($path,strlen($dir))));
            }
        }
        $zip->close();
        return $file;
...
if (!function_exists('addNamespace')) {
    function addNamespace($prefix,$paths=[]){
